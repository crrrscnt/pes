...
def build_problem(distance, atom_name, basis="sto3g"):
    ...
    for spin in spin_values:
        try:
            ...
            problem = driver.run()
            if spin != 0:
                print(f"Note: Using spin={spin} for {atom_name}H system", file=sys.stderr)
            return problem
            ...
            if is_spin_error:
                if spin == spin_values[-1]:
                    ...
                    print(f"Spin {spin} failed, trying next value...", file=sys.stderr)
                    continue
            else:
...
def create_vqe_local(mapper, num_spatial_orbitals, num_particles,
    ...
    print(f"Debug: Creating VQE components with: num_spatial_orbitals={num_spatial_orbitals}, "
...
def main():
    # Ожидаем аргументы: distance optimizer ansatz mapper atom_name
    if len(sys.argv) < 6:
        print(json.dumps({"error": "usage: worker.py <distance> <optimizer> <ansatz> <mapper> <atom_name>"}))
        return

    distance = float(sys.argv[1])
    optimizer_name = sys.argv[2]
    ansatz_reps_str = sys.argv[3]
    mapper_name = sys.argv[4]
    atom_name = sys.argv[5]

    try:
        try:
            ansatz_reps = int(ansatz_reps_str)
        except ValueError:
            ansatz_reps = 1
            print(f"Warning: Invalid reps '{ansatz_reps_str}', defaulting to 1",
                  file=sys.stderr)

        print(f"Debug: Computing point at {distance:.3f} Å with {optimizer_name} optimizer, "
              f"{mapper_name} mapper", file=sys.stderr)
        problem = build_problem(distance, atom_name)
        print(f"Debug: Problem built with {problem.num_spatial_orbitals} spatial orbitals, "
        ...
        out = {"distance": distance, "vqe": vqe_energy, "numpy": np_energy,
        ...
        print(json.dumps(
            {"distance": distance, "error": str(e), "traceback": tb}))
