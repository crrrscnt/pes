...
def build_problem(distance, atom_a, atom_b, basis="sto3g"):
    """Создает квантовую задачу для двухатомной молекулы."""
    spin_values = [0, 1, 3]
    last_error = None
    ...
    for spin in spin_values:
        try:
            driver = PySCFDriver(
                atom=f"{atom_a} 0 0 0; {atom_b} 0 0 {distance}",
                basis=basis,
                charge=0,
                spin=spin,
            ...
            problem = driver.run()
            if spin != 0:
                print(f"Note: Using spin={spin} for {atom_a}{atom_b} system",
                      file=sys.stderr)
            return problem
...
def compute_single_point(distance, optimizer_name, mapper_name,
                         atom_a, atom_b, ansatz_reps=1):
